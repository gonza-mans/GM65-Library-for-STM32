...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  //HAL_Delay(1000);
  gm65_init(&QRScanner, &huart3, &htim9);
  QRScanner.qrDataReady=0;
  /* USER CODE END 2 */
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart){
	if(huart->Instance == USART3){
		gm65_uart_TX_handler(&QRScanner, &huart3, &htim9);
		}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	if(huart->Instance == USART3){
		gm65_uart_RX_handler(&QRScanner, &huart3, &htim9);
	}
}

void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){

	gm65_scan(&QRScanner, &huart3);
}


void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	if(htim->Instance ==TIM9){
		gm65_Tim_handler(&QRScanner, &huart3, &htim9);
	}
}
